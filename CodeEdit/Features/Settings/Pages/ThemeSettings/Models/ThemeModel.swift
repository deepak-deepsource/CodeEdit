...
final class ThemeModel: ObservableObject {
  ...
  func loadThemes() throws {  // swiftlint:disable:this function_body_length
    ...
    let content = try filemanager.contentsOfDirectory(atPath: url.path).filter {
      $0.contains(".json")
    }
    ...
    try content.forEach { file in
      ...
      if var theme = try load(from: fileURL) {
        ...
        guard
          let terminalColors = try theme.terminal.allProperties() as? [String: Theme.Attributes],
  ...
  private func loadBundledThemes() throws {
    ...
    for themeName in bundledThemeNames {
      ...
      try prettyJSON.write(
        to: themesURL.appendingPathComponent("\(themeName).json"), options: .atomic)
    }
  }
