...
struct TerminalEmulatorView: NSViewRepresentable {
  ...
  init(url: URL, shellType: String? = nil, onTitleChange: @escaping (_ title: String) -> Void) {
    ...
    self._terminal = State(
      initialValue: TerminalEmulatorView.lastTerminal[url.path] ?? .init(frame: .zero))
  }
  ...
  func setupSession() {
    ...
    if TerminalEmulatorView.lastTerminal[url.path] == nil {
      ...
      terminal.startProcess(
        executable: shell, environment: terminalEnvironment, execName: shellIdiom)
      terminal.font = font
      ...
      terminal.nativeBackgroundColor =
        terminalSettings.useThemeBackground ? backgroundColor : .clear
