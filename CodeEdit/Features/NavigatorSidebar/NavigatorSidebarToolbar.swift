...
struct NavigatorSidebarToolbar: View {
  ...
  func iconsView(size: CGSize) -> some View {
    let layout =
      alignment == .top ? AnyLayout(HStackLayout(spacing: 0)) : AnyLayout(VStackLayout(spacing: 10))
    layout {
      ForEach(icons) { icon in
        makeIcon(named: icon.imageName, title: icon.title, id: icon.id, sidebarWidth: size.width)
          .opacity(
            draggingItem?.imageName == icon.imageName && hasChangedLocation
  ...
  struct NavigatorToolbarButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
      configuration.label
        ...
        .foregroundColor(
          id == selection ? .accentColor : configuration.isPressed ? .primary : .secondary
        )
  ...
  private struct NavigatorSidebarDockIconDelegate: DropDelegate {
    ...
    func dropEntered(info: DropInfo) {
      ...
      guard item != current, let current = current,
        ...
        let toIndex = icons.firstIndex(of: item)
      else { return }
      ...
      if icons[toIndex] != current {
        icons.move(
          fromOffsets: IndexSet(integer: from), toOffset: toIndex > from ? toIndex + 1 : toIndex)
      }
    }
