...
final class ProjectNavigatorViewController: NSViewController {
  ...
  private var content: [CEWorkspaceFile] {
    ...
    guard let root = try? workspace?.workspaceFileManager?.getFile(folderURL.path) else {
      return []
    }
    return [root]
  }
  ...
  private func onItemDoubleClicked() {
    guard let item = outlineView.item(atRow: outlineView.clickedRow) as? CEWorkspaceFile else {
      return
    }
...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
  ...
  func outlineView(
    ...
    guard let pasteboardItems = info.draggingPasteboard.readObjects(forClasses: [NSURL.self]) else {
      return false
    }
    ...
    for fileItemURL in fileItemURLS {
      ...
      var srcFileItem: CEWorkspaceFile? = try? workspace?.workspaceFileManager?.getFile(
        fileItemURL.path)
...
extension ProjectNavigatorViewController: NSOutlineViewDelegate {
    ...
    -> NSView?
  {
    ...
    return ProjectNavigatorTableViewCell(
      frame: frameRect, item: item as? CEWorkspaceFile, delegate: self)
  }
  ...
  func outlineView(_ outlineView: NSOutlineView, itemForPersistentObject object: Any) -> Any? {
    guard let id = object as? CEWorkspaceFile.ID,
      let item = try? workspace?.workspaceFileManager?.getFile(id)
    else { return nil }
    return item
  }
