...
final class ProjectNavigatorMenu: NSMenu {
  ...
  private func setupMenu() {
    ...
    let openExternalEditor = menuItem(
      "Open with External Editor", action: #selector(openWithExternalEditor))
    ...
    let duplicate = menuItem(
      "Duplicate \(item.isFolder ? "Folder" : "File")", action: #selector(duplicate))
  ...
  private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
    ...
    func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
      ...
      if type.conforms(to: UTType(filenameExtension: "xcassets")!) {
        primaryItems.append(
          NSMenuItem(title: "Asset Catalog Document", action: nil, keyEquivalent: ""))
      }
      if type.conforms(to: UTType(filenameExtension: "xib")!) {
        primaryItems.append(
          NSMenuItem(title: "Interface Builder XIB Document", action: nil, keyEquivalent: ""))
      }
  ...
  private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
    ...
    sourceControlMenu.addItem(
      withTitle: "Mark Selected Files as Resolved", action: nil, keyEquivalent: "")
