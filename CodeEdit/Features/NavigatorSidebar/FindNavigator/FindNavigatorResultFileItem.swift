...
struct FindNavigatorResultFileItem: View {
  ...
  @ViewBuilder
  private func foundLineResult(_ lineContent: String?, keywordRange: Range<String.Index>?)
    -> some View
  {
    ...
    DisclosureGroup(isExpanded: $isExpanded) {
      ...
      HStack {
        ...
        Text(fileItem.fileName)
          ...
          + Text(
            fileItem.url.path.replacingOccurrences(
              of: state.workspace.fileURL?.path ?? "", with: "")
          )
