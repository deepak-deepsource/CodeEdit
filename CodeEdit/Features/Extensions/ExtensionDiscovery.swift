...
final class ExtensionDiscovery: ObservableObject {
  ...
  private static var dbURL = URL.libraryDirectory
    .appending(
      path: "Preferences/com.apple.LaunchServices/com.apple.LaunchServices.SettingsStore.sql")
  ...
  private func discover() async {
    ...
    do {
      let sequence = try AppExtensionIdentity.matching(
        appExtensionPointIDs: Self.endPointIdentifier)
  ...
  private func updateExtensions(
    endpoints: [AppExtensionIdentity], shouldRestartExisting: Bool = false
  ) async {
  ...
  private func activateUnapprovedExtensions(with identifiers: [(bundleID: String, devID: String)])
    async throws
  {
    let dbQueue = try DatabaseQueue(path: Self.dbURL.path())
    ...
    return try await dbQueue.write { table in
      try identifiers.map { identifier in
        SettingsStoreRecord(
          ...
          identifier:
            "\(Bundle.main.bundleIdentifier!)::\(Self.endPointIdentifier):\(identifier.bundleID):\(identifier.devID)",
