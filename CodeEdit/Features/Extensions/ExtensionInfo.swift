...
struct ExtensionInfo: Identifiable, Hashable {
  ...
  init(endpoint: AppExtensionIdentity) async throws {
    ...
    let process = try await AppExtensionProcess(
      configuration: .init(appExtensionIdentity: endpoint))
...
extension ExtensionInfo {
  ...
  private static func getAvailableFeatures(_ connection: NSXPCConnection) async throws
    -> [ExtensionKind]
  {
    let encodedAvailableFeatures = try await connection.withContinuation {
      (service: XPCWrappable, continuation) in
  ...
  private static func getBundleURL(_ connection: NSXPCConnection) async throws -> URL {
    let bundleURLEncoded = try await connection.withContinuation {
      (service: XPCWrappable, continuation) in
