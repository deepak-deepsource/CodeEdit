...
enum FuzzySearch {
  ...
  private static func score(tokens: [String], url: URL) -> Double {
    ...
    for token in tokens {
      ...
      if !ranges.isEmpty {
        ...
        let levenshteinScore =
          Double(levenshteinDistance(from: String(token), to: text)) / Double(text.count)
  ...
  private static func proximityScoreForRanges(_ ranges: [Range<String.Index>], text: String)
    -> Double
  {
    ...
    for index in 1..<sortedRanges.count {
      ...
      let distance =
        currentRange.lowerBound.utf16Offset(in: text)
  ...
  private static func levenshteinDistance(from sourceString: String, to targetString: String) -> Int
  {
    ...
    var distanceMatrix = Array(
      repeating: Array(repeating: 0, count: targetCount + 1), count: sourceCount + 1)
...
extension String {
  ...
  func ranges(of searchString: String) -> [Range<String.Index>] {
    ...
    while let range = self[searchStartIndex..<endIndex].range(
