...
final class CEWorkspaceFileManager {
  ...
  private func setup() {
    ...
    fsEventStream = DirectoryEventStream(directory: self.folderUrl.path) {
      [weak self] path, event, deepRebuild in
    ...
    let workspaceFile = CEWorkspaceFile(
      url: self.folderUrl, children: workspaceFiles.sortItems(foldersOnTop: true))
