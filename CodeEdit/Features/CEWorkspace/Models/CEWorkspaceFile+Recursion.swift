...
extension CEWorkspaceFile {
  ...
  func flattenedChildren(withDepth depth: Int, ignoringFolders: Bool) -> [CEWorkspaceFile] {
    ...
    children?.forEach { child in
      childItems.append(
        contentsOf: child.flattenedChildren(
          withDepth: depth - 1,
  ...
  func appearanceWithinChildrenOf(searchString: String, ignoredStrings: [String] = [".", "~"])
    -> Int
  {
    var count = 0
  ...
  func childrenSatisfying(searchString: String, ignoredStrings: [String] = [".", "~"])
    -> [CEWorkspaceFile]
  {
