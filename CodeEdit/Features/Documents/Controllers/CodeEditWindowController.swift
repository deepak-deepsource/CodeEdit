...
final class CodeEditWindowController: NSWindowController, NSToolbarDelegate, ObservableObject {
  ...
  init(window: NSWindow, workspace: WorkspaceDocument) {
    ...
    observers = [
      splitViewController.splitViewItems.first!.observe(
        \.isCollapsed,
        changeHandler: { [weak self] item, _ in
          self?.navigatorCollapsed = item.isCollapsed
        }),
      splitViewController.splitViewItems.last!.observe(
        \.isCollapsed,
        changeHandler: { [weak self] item, _ in
          self?.navigatorCollapsed = item.isCollapsed
        }),
  ...
  private func setupSplitView(with workspace: WorkspaceDocument) {
    ...
    let splitVC = CodeEditSplitViewController(
      workspace: workspace, feedbackPerformer: feedbackPerformer)
  ...
  @IBAction func closeActiveTabGroup(_ sender: Any) {
    if workspace?.tabManager.tabGroups.findSomeTabGroup(
      except: workspace?.tabManager.activeTabGroup) == nil
    {
