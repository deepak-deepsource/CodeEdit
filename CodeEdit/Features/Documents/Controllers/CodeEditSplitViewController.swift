...
extension CGFloat {
  fileprivate static let snapWidth: CGFloat = 272

  fileprivate static let minSnapWidth: CGFloat = snapWidth - 10
  fileprivate static let maxSnapWidth: CGFloat = snapWidth + 10
}
...
final class CodeEditSplitViewController: NSSplitViewController {
  ...
  override func viewWillAppear() {
    ...
    if let firstSplitView = splitViewItems.first {
      firstSplitView.isCollapsed =
        workspace.getFromWorkspaceState(
          .navigatorCollapsed
        ) as? Bool ?? false
    }
  ...
  private func insertToolbarItemIfNeeded() {
    guard
      !(view.window?.toolbar?.items.contains(where: {
  ...
  private func removeToolbarItemIfNeeded() {
    guard
      let index = view.window?.toolbar?.items.firstIndex(
  ...
  func hideInspectorToolbarBackground() {
    let controller = self.view.window?.perform(Selector(("titlebarViewController")))
      .takeUnretainedValue()
