    ...
    func lineContainsSearchTerm(line rawLine: String, term searchterm: String) -> Bool {
      ...
      if findMode == .Text {
        ...
        for appearance in appearances {
          ...
          var endsWith = false
          if appearanceString.hasPrefix(searchterm) || !appearanceString.first!.isLetter
            || !appearanceString.character(at: 2).isLetter
          {
            startsWith = true
          }
          if appearanceString.hasSuffix(searchterm) || !appearanceString.last!.isLetter
...
extension String {
  ...
  func appearancesOfSubstring(substring: String, toLeft: Int = 0, toRight: Int = 0) -> [Range<
    String.Index
  >] {
