...
extension NSEvent.Publisher {
  fileprivate final class Subscription<S: Subscriber> where S.Input == NSEvent, S.Failure == Never {
    ...
    init(scope: Scope, matching: NSEvent.EventTypeMask, subscriber: S) {
      switch scope {
      case .local:
        self.monitor = NSEvent.addLocalMonitorForEvents(matching: matching) {
