  ...
  func postJSON<T>(
    ...
    do {
      data = try JSONSerialization.data(
        withJSONObject: params, options: JSONSerialization.WritingOptions())
    } catch {
    ...
    let task = session.uploadTask(with: request, fromData: data) { data, response, error in
      ...
      if let error {
        ...
        if let data {
          do {
            let JSON =
              try JSONSerialization.jsonObject(with: data, options: .mutableContainers) as? T
            completion(JSON, nil)
          } catch {
            completion(nil, error)
          }
    ...
    func postJSON<T>(
      ...
      return try JSONSerialization.jsonObject(with: responseTuple.0, options: .mutableContainers)
        as? T
    }
  #endif
