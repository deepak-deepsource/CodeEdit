...
extension GitHubAccount {
  ...
  func postGistFile(
    ...
    let router = GitHubGistRouter.postGistFile(
      configuration, description, filename, fileContent, publicAccess)
  ...
  func patchGistFile(
    ...
    let router = GitHubGistRouter.patchGistFile(
      configuration, id, description, filename, fileContent)
    let decoder = JSONDecoder()
